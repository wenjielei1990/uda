...
def filter_unicode(st):
    """Filter out unicode characters not in ASCII printable."""
...
class TfIdfWordRep(EfficientRandomGen):
    ...
    def __init__(self, token_prob, data_stats):
        ...
        for key, value in tf_idf_items:
            self.tf_idf_keys.append(key)
            self.tf_idf_values.append(value)
    ...
    def get_replace_prob(self, all_words):
        ...
        replace_prob = []
        for word in all_words:
            replace_prob.append(cur_tf_idf[word])
    ...
    def __call__(self, example):
        ...
        if show_example:
            logging.info("before tf_idf_unif aug: %s",
        ...
        if show_example:
            ...
            if example.text_b:
                all_words += example.word_list_b
            logging.info("after tf_idf_unif aug: %s",
                filter_unicode(" ".join(all_words)))
        return example
    ...
    def reset_token_list(self):
        ...
        for idx in token_list_idx:
            self.token_list.append(self.tf_idf_keys[idx])
        self.token_ptr = len(self.token_list) - 1
        logging.info("sampled token list: %s",
